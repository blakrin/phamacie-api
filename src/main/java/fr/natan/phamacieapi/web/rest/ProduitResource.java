...
/**
 * REST controller for managing {@link fr.natan.phamacieapi.domain.Produit}.
 */
@RestController
@RequestMapping("/api")
public class ProduitResource {
    ...
    @GetMapping("/produits/search/{searchValue}")
    public ResponseEntity<Page<ProduitDTO>> getAllProduitsByCodeOrName(Pageable pageable, @PathVariable String searchValue) {
        log.debug("REST request to get a page of Produits");
        Page<ProduitDTO> page = produitService.findAllByCodeOrName(pageable, searchValue);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page);
    }
